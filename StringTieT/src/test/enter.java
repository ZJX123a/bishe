...
public class enter {
	public static void print_message(kmerHash kh) {
		...
		kh.kmer_length = read;
		System.out.println("请输入reads测序方式，1：(2)-><-(1)   2:(1)-><-(2)");
		read = scan.nextInt();
		while (read != 1 && read != 2) {
			System.out.println("您输入的数据不合法，请重新输入！");
			read = scan.nextInt();
	...
	public static void main(String args[]) throws IOException {
		...
		print_message(kh);
		System.out.println(kh.kmer_length);
		int count = 0;
		...
		FileOutputStream out = new FileOutputStream("D:/CESHI.fasta");
		int count_sum = 0;
			...
			if (!used_kmers_plus.containsKey(kh.list.get(i).getKey())) {
				...
				if (sg.init_trunk(kh, kh.list.get(i).getKey(), node_jihe, sg)) {
					...
					if (sg.node_set.size() == 1) {
						...
						p.println(">The_" + graph_count + "_Graph_" + 1 + "_Transcript_" + count_sum);
						// count_result++;
						count_sum++;
						...
						for (Map.Entry<Vector<Integer>, Float> mapping : list_path) {
							...
							float fenmu = 0;
							int self = 0;
							for (int k = 0; k < path_highest.size(); k++) {
								int node = path_highest.get(k);
								fenzi = 0;
								fenmu = 0;
								// if (nodes[k] != 0) {
								self = node_cov[node];
								...
								for (int m = 0; m < sg.node_set.size(); m++) {
									if (edges_ter[node][m] != 0) {
										fenmu += (float) 1 / (edges_ter[node][m]);
										self -= edges_ter[node][m];
									}
								}
								// 处理父节点

								for (int m = 0; m < sg.node_set.size(); m++) {
									if (edges_ter[m][node] != 0) {
										fenzi += (float) 1 / (edges_ter[m][node]);
										self -= edges_ter[m][node];
									}
								}
								bv[node] = (float) (((float) (1.0 / self) + fenzi) / ((float) (1.0 / self) + fenmu));
								// }
							}
							...
							// for(int k=0;k<sg.node_set.size();k++){
							// for(int m=0;m<sg.node_set.size();m++){
							// System.out.print(edges_ter[k][m]+" ");
							// }
							// System.out.println();
							// }
							// System.out.println("path_highest:" +
							...
							max_flow = fn.max_flow(path_highest.get(0), path_highest.lastElement(), edges_ter, node_cov,
									kh, bv, sg, path_highest);
							if (max_flow > 0) {
								p.println(
										">The_" + graph_count + "_Graph_" + count_result + "_Transcript_" + count_sum);
								count_result++;
								count_sum++;
								p.println(path_str);
								// p.println("最大流为：" + max_flow);
